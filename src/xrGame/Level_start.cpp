...
bool CLevel::net_start3()
{
	...
	if (m_caServerOptions.size())
	{
			...
			!strstr(m_caClientOptions.c_str(), "psw="))
		{
			...
			string4096 tmp;
			xr_sprintf(
				tmp, "%s/psw=%s", m_caClientOptions.c_str(), PasswordStr);
			m_caClientOptions = tmp;
		};
	};
	...
	if (strstr(m_caClientOptions.c_str(), "/cdkey="))
	{
		string64 CDKey;
		const char* start =
			strstr(m_caClientOptions.c_str(), "/cdkey=") + xr_strlen("/cdkey=");
		sscanf(start, "%[^/]", CDKey);
		string128 cmd;
...
bool CLevel::net_start6()
{
	...
	else
	{
			...
			!psNET_direct_connect && !g_dedicated_server)
		{
			...
			m_bConnectResult) // if (map_data.m_name == "") - level not loaded,
		                      // see CLevel::net_start_client3
		{
