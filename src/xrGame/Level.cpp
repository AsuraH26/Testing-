...
#endif // PROFILE_CRITICAL_SECTIONS
{
	...
	eChangeTrack =
		g_pEventManager->Event.Handler_Attach("LEVEL:PlayMusic", this);
	eEnvironment =
		g_pEventManager->Event.Handler_Attach("LEVEL:Environment", this);
...
void CLevel::PrefetchSound(LPCSTR name)
{
	...
	if (it == sound_registry.end())
		sound_registry[snd_name].create(
			snd_name.c_str(), st_Effect, sg_SourceType);
}
...
void CLevel::cl_Process_Event(u16 dest, u16 type, NET_Packet& P)
{
	...
	if (type != GE_DESTROY_REJECT)
	{
		if (type == GE_DESTROY)
		{
			...
			//			{
			// = GameObject.cpp (210)
			//				Msg( "! ERROR (Level): GE_DESTROY arrived to
			// object[%d][%s], that has parent[%d][%s], frame[%d]",
			// GO->ID(), GO->cNameSect().c_str(),
			// GO->H_Parent()->ID(), GO->H_Parent()->cName().c_str(),
			// Device.dwFrame );
			//			}
		}
		GO->OnEvent(P, type);
...
void CLevel::MakeReconnect()
{
	if (!g_pEventManager->Event.Peek("KERNEL:disconnect"))
	{
		...
		g_pEventManager->Event.Defer(
			"KERNEL:start", size_t(server_options), size_t(client_options));
	}
}
...
void CLevel::OnFrame()
{
	...
	if (!g_dedicated_server)
	{
		...
		if (Device.dwPrecacheFrame == 0)
		{
			...
			//(fastdelegate::FastDelegate0<>(m_game_task_manager,&CGameTaskManager::UpdateTasks));
			// else
	...
	if (!g_dedicated_server && psDeviceFlags.test(rsStatistic))
	{
		...
		if (!psNET_direct_connect)
		{
			if (IsServer())
			{
				...
				F->OutNext("IN:  %4d/%4d (%2.1f%%)", S->bytes_in_real,
					S->bytes_in,
				...
				F->OutNext(
					"SPS/Sended : %4d/%4d", S->dwBytesPerSec, S->dwBytesSended);
				...
				struct net_stats_functor
				{
					...
					void operator()(IClient* C)
					{
						...
						F->OutNext("0x%08x: P(%d), BPS(%2.1fK), MRR(%2d), "
								   "MSR(%2d), Retried(%2d), Blocked(%2d)",
			...
			if (IsClient())
			{
				...
				F->SetColor(color_xrgb(255, 255, 255));
				F->OutNext("BReceivedPs(%2d), BSendedPs(%2d), Retried(%2d), "
						   "Blocked(%2d)",
				...
				if (!pStatGraphR)
				{
					...
					pStatGraphR->SetRect(
						50, 700, 300, 68, 0xff000000, 0xff000000);
				...
				pStatGraphR->AppendItem(
					float(net_Statistic.getBPS()), 0xff00ff00, 0);
				F->OutSet(20.f, 700.f);
				F->OutNext("64 KBS");
...
void CLevel::OnRender()
{
	...
	if (ai().get_level_graph() &&
		dynamic_cast<CLevelGraph*>(&ai().level_graph()))
	...
	if (bDebug)
	{
		...
		if (Server)
			UI().Font().pFontStat->OutNext(
				"Client Objects:      [%d]", Server->GetEntitiesNum());
		UI().Font().pFontStat->OutNext(
			"Server Objects:      [%d]", Objects.o_count());
		UI().Font().pFontStat->OutNext(
			"Interpolation Steps: [%d]", Level().GetInterpolationSteps());
		if (Server)
		{
			UI().Font().pFontStat->OutNext(
				"Server updates size: [%d]", Server->GetLastUpdatesSize());
		}
...
void CLevel::OnEvent(EVENT E, u64 P1, u64 /**P2/**/)
{
	if (E == eEntitySpawn)
	{
		...
		Level().g_cl_Spawn(
			Name, 0xff, M_SPAWN_OBJECT_LOCAL, Fvector().set(0, 0, 0));
	}
...
void CLevel::AddObject_To_Objects4CrPr(CGameObject* pObj)
{
	if (!pObj)
		return;
	for (OBJECTS_LIST_it OIt = pObjects4CrPr.begin();
		 OIt != pObjects4CrPr.end(); OIt++)
	{
		if (*OIt == pObj)
			return;
	}
...
void CLevel::AddActor_To_Actors4CrPr(CGameObject* pActor)
{
	...
	if (!smart_cast<CActor*>(pActor))
		return;
	for (OBJECTS_LIST_it AIt = pActors4CrPr.begin(); AIt != pActors4CrPr.end();
		 AIt++)
	{
		if (*AIt == pActor)
			return;
...
void CLevel::RemoveObject_From_4CrPr(CGameObject* pObj)
{
	...
	OBJECTS_LIST_it OIt =
		std::find(pObjects4CrPr.begin(), pObjects4CrPr.end(), pObj);
	...
	OBJECTS_LIST_it AIt =
		std::find(pActors4CrPr.begin(), pActors4CrPr.end(), pObj);
...
void CLevel::make_NetCorrectionPrediction()
{
	...
	for (u32 i = 0; i < m_dwNumSteps; i++)
	{
		...
		for (OBJECTS_LIST_it AIt = pActors4CrPr.begin();
			 AIt != pActors4CrPr.end(); AIt++)
		{
...
u8 CLevel::GetDayTime()
{
	...
	u32 hours;
	GetGameDateTime(
		dummy32, dummy32, dummy32, hours, dummy32, dummy32, dummy32);
	VERIFY(hours < 256);
	return u8(hours);
}
...
{
	return (
		float(s64(GetEnvironmentGameTime() % (24 * 60 * 60 * 1000))) / 1000.f);
}

void CLevel::GetGameDateTime(u32& year, u32& month, u32& day, u32& hours,
	u32& mins, u32& secs, u32& milisecs)
{
...
void CLevel::SetGameTimeFactor(
	ALife::_TIME_ID GameTime, const float fTimeFactor)
{
	game->SetGameTimeFactor(GameTime, fTimeFactor);
}

void CLevel::SetEnvironmentGameTimeFactor(
	u64 const& GameTime, float const& fTimeFactor)
{
	if (!game)
		return;
